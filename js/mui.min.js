...
mui.plusReady(function(){ 
 ...
 console.log('判断是否支持沉浸式'+isImmersedStatusbar);
 ...
 if (isImmersedStatusbar) {
   ...
   if (StatusbarHeight>0) {
	   ...
	   console.log('通知栏高StatusbarHeight'+StatusbarHeight);
	   if (h('.mui-navbar').eq(0).length>0) {
		//	console.log(h('.mui-navbar .mui-bar')); 
		h('.mui-bar').css({ 'paddingTop':12+'px'});
	   }
	   if (h('header').eq(0).length>0) {
		   	h('.mui-bar .mui-action-back').eq(0).css({ 'top':12+'px'}); 
		      console.log('h(header).length:'+h('header').length);
		   	h('.mui-bar .mui-title').eq(0).css({'lineHeight':66+'px'}); 
